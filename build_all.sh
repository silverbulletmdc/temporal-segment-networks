...
[[ -d build ]] || mkdir build
cd build
cmake -D WITH_CUDA=ON -DWITH_OPENGL=ON -D OPENCV_ENABLE_NONFREE=ON -D CMAKE_BUILD_TYPE=RELEASE -D WITH_TBB=ON  -D WITH_V4L=ON -D BUILD_TIFF=ON -D OPENCV_EXTRA_MODULES_PATH=../../opencv_contrib/modules ..
if make -j32 ; then
    cp lib/cv2.so ../../../
